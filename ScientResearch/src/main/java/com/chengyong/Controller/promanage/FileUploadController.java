...
public class FileUploadController {
    ...
    @RequestMapping("/projectresFile")
    public Map<String,Object> projectresFile(MultipartFile file, HttpServletRequest request) throws IOException {
        Map<String,Object> map = new HashMap<>();
        String fileType = file.getOriginalFilename().substring(file.getOriginalFilename().lastIndexOf(".")+1,file.getOriginalFilename().length());
        //获取项目本身的地址根目录
        String path = PUBLIC_ATTRIBUTE.UPLOAD_PATH_PRORESULT;
        ...
        in.close();
        out.close();
        request.getSession().setAttribute("proresult_path",path+"/"+filename);
        map.put("info", PUBLIC_ATTRIBUTE.UPLOAD_SUCCESS);
        return map;
    }

    @RequestMapping("/patentFile")
    public Map<String,Object> patentFile(MultipartFile file, HttpServletRequest request) throws IOException {
        Map<String,Object> map = new HashMap<>();
        String fileType = file.getOriginalFilename().substring(file.getOriginalFilename().lastIndexOf(".")+1,file.getOriginalFilename().length());
        //获取项目本身的地址根目录
        String path = PUBLIC_ATTRIBUTE.UPLOAD_PATH_PATENT;
        ...
        in.close();
        out.close();
        request.getSession().setAttribute("patent_path",path+"/"+filename);
        map.put("info", PUBLIC_ATTRIBUTE.UPLOAD_SUCCESS);
        return map;
    }

    @RequestMapping("/messageFile")
    public Map<String,Object> messageFile(MultipartFile file, HttpServletRequest request) throws IOException {
        Map<String,Object> map = new HashMap<>();
        String fileType = file.getOriginalFilename().substring(file.getOriginalFilename().lastIndexOf(".")+1,file.getOriginalFilename().length());
        //获取项目本身的地址根目录
        String path = PUBLIC_ATTRIBUTE.UPLOAD_PATH_MESSAGE;
        ...
        in.close();
        out.close();
        request.getSession().setAttribute("message_path",path+"/"+filename);
        map.put("info", PUBLIC_ATTRIBUTE.UPLOAD_SUCCESS);
        return map;
    }
}
